...
void Pipeline::cycle(void) {
	...
	// NOTE: Technically, data hazards are detected in the Decode stage. If a data hazard is detected, 
	// at the end of the cycle we write 0's (NOP) to the pipeline register so that a NOP will be generated 
	//in the EXEC stage in the next cycle. 
	// Doing the check here does a dependency check on the instructions in the previous cycle 
	//(we haven't advanced the instructions in the pipeline yet). If a dependency exist in the previous cycle, 
	//we stall the pipeline in this cycle.
	...
	else 
	{
		if(forwarding)
        {
			if(pipeline[MEM].inst->type!=LW && (pipeline[MEM].inst->dest ==  pipeline[DECODE].inst->src1 || pipeline[MEM].inst->dest ==  pipeline[DECODE].inst->src2))
            {
            	pipeline[EXEC].addInstruction(pipeline[DECODE].inst);	
				dependencyDetected=false;
            } 
        	
            else if(pipeline[MEM].inst->type!=LW && (pipeline[WB].inst->dest ==  pipeline[DECODE].inst->src1 || pipeline[WB].inst->dest ==  pipeline[DECODE].inst->src2))
             {
             	pipeline[EXEC].addInstruction(pipeline[DECODE].inst);
			 	dependencyDetected=false;
             }
...
bool Pipeline::hasDependency(void) {
	...
	for(int i = EXEC; i < WB; i++) {
		...
		if( (pipeline[i].inst->dest != -1) && 
			...
			if(forwarding && pipeline[WB].inst->type==LW)
			{
	 			return false;
			}
			return true;
